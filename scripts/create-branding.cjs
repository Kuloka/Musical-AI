...
async function main() {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="2560" height="1280" viewBox="0 0 2560 1280">
    ...
    <text x="1280" y="623" text-anchor="middle" font-family="Segoe UI,Arial,sans-serif" font-size="108" font-weight="600" letter-spacing="-4" fill="#e8e8e8">Musical AI</text>
  ...
  fs.writeFileSync(path.join(directory, 'musical-banner.svg'), svg);
  await sharp(Buffer.from(svg)).png().toFile(path.join(directory, 'musical-banner.png'));
  await sharp(Buffer.from(svg)).resize(1280, 640).png().toFile(path.join(directory, 'musical-github.png'));
  const icon = fs.readFileSync(path.join(__dirname, '..', 'resources', 'musical-logo.svg'), 'utf8');
  await sharp(Buffer.from(icon)).resize(1024,1024).png().toFile(path.join(directory, 'musical-icon.png'));
