...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'multimind-native-smoke-'));
os.homedir = () => temp;
app.setPath('userData', path.join(temp, 'profile'));
const unpacked = process.platform === 'win32' ? 'win-unpacked/resources' : process.platform === 'linux' ? 'linux-unpacked/resources' : `mac${process.arch === 'arm64' ? '-arm64' : ''}/MultiMind.app/Contents/Resources`;
