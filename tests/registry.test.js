...
test('registry download validates names, model layers and integrity metadata', async () => {
  await assert.rejects(registryAsset('multimind:../bad:model', new AbortController().signal), /Invalid model name/);
  ...
  try {
    ...
    const asset = await registryAsset('multimind:qwen2.5:0.5b', new AbortController().signal);
    ...
    await assert.rejects(registryAsset('multimind:qwen2.5:0.5b'), /additional components/);
    manifest = { layers: [{ mediaType: 'application/vnd.ollama.image.model', digest: '../file', size: 123 }] };
    await assert.rejects(registryAsset('multimind:qwen2.5:0.5b'), /Invalid model manifest/);
