...
test('Real stdio MCP discovery, project reads, traversal refusal, persistence and disable',async t=>{
  const dir=await fs.mkdtemp(path.join(os.tmpdir(),'musical-mcp-'));
...
test('Streamable HTTP MCP discovery and execution',async t=>{
  ...
  const dir=await fs.mkdtemp(path.join(os.tmpdir(),'musical-http-'));const plugins=createPlugins(dir,dir);
